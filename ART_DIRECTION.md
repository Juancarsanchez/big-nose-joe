...
- ambas paredes tienen un billón de puntos de resistencia;
...
14. `Layer59_Adaptations`: esponjas y catapultas, montadas como cuerpo y accesorio reemplazables.
...
- `pawn_empty.png` reutilizado por los púgiles, con guante, cinta, venda de proteína, parche bronquial, marca de combo y venda de uranio añadidos como capas hijas independientes;
- `leukocyte_ram.png`, `leukocyte_elephant.png`, `leukocyte_supersaiyan.png` y `plasma_cannon.png`: cuatro siluetas de extracción independientes, siempre apoyadas en el suelo; la carrera, el proyectil de plasma y el Kamehameha viven en la capa de efectos;
...
Una bola apelmazada natural representa seis granos vecinos y solo la transporta un casco azul después de tratarla. El contador que intenta crearla baja de 36 a 12 aterrizajes conforme crece la extracción automática: una producción más agresiva genera más trabajo de casco, con un límite simultáneo para que el sistema no absorba todo el juego. `APELMAZADO INTELIGENTE` no crea ese obstáculo: durante el transporte, los peones básicos muestran una bola de carga ligeramente mayor por cada paquete de 3, 5, 8 o 12 granos. Las piezas agrupadas siguen siendo datos individuales, de modo que se conserva su valor y no aparece otro tipo de pedrusco en el suelo.

El peón básico, el carrito, el `MUGIDÓFILO DE CARGA` y el `EXPRESO LEUCOCITARIO` son transporte puro: el carrito evoluciona 12 → 60 → 300 → 1.500 y el Mugidófilo 10.000 → 150.000 → 6,5 millones → 100 millones → 10.000 millones. Ninguno daña paredes ni bloques. Estas mejoras no crean unidades nuevas: se leen mediante el número de carga y ligeros cambios de escala del edificio receptor. El espacio físico crece 1.000 → 5.000 → 25.000 → 100.000 → 2 millones → 10 millones → 100 millones → 1.000 millones → 10.000 millones → 100.000 millones; el silo y la planta crecen ligeramente en sus últimas evoluciones y conservan su propio rótulo y cifra exacta.
