...
Las fases ya no avanzan por tiempo ni por entregas. Joe empieza al 90% y cada tramo se desbloquea cuando el jugador logra reducir su colocón hasta 70%, 52%, 34% y 18%. Cada umbral provoca inmediatamente la nueva barbaridad y añade su temporizador al repertorio permanente. El medidor principal de colocón ocupa el centro superior: picar la pared lo hace bajar al instante, muestra las unidades reales extraídas y pulsa en azul; almacenar ese mismo polvo no aplica la reducción dos veces. La versión 19 inicia una run limpia: no regala porteadores, parte de un cajón de 500 unidades y descarta los guardados de prototipos anteriores.

## Cocaína, apelmazado y transporte

La escala es universal desde cinco unidades: una unidad lógica de cocaína equivale a un píxel de pantalla de superficie en cualquier estado visible. Existe una excepción de legibilidad para el polvo suelto: las cantidades de 1, 2, 3 o 4 se dibujan siempre con cuatro píxeles, aunque conservan intacto su valor lógico. Desde 5 la progresión vuelve a ser exactamente lineal; un impacto de 10 genera diez píxeles y una carga de 100 conserva cien durante la caída, en la pila, sobre el portador y durante la descarga. Las caídas son nubes anchas de copos y la recogida distribuye las motas a lo largo de todo el arco, sin columnas ni cadenas rígidas. No existen bolas con un valor oculto. Cada descarga engorda la superficie bajo su punto real de aterrizaje; las zonas cercanas se funden y las lejanas forman lomas separadas con valles. Los pedruscos son la segunda excepción visual: mantienen exactamente seis unidades lógicas y su composición, pero recuperan un sprite de obstáculo suficientemente grande para poder verlos, romperlos y recogerlos. El tabique actúa como límite duro y ninguna masa puede cruzarlo.

El hueco inicial admite 75K de polvo, pero no se presenta todavía como una fosa ni enseña instrumentación que el jugador no ha construido. Si llega al límite, se detiene todo el minado —manual y automático— mientras los porteadores siguen retirando nieve. La tecnología `FOSA SUBMUCOSA` desplaza el almacén, abre volumen profundo hasta 600K y hace aparecer tanto al `LEUCOTOPÓGRAFO DE 1927` como su medidor en tiempo real. No es un requisito impuesto por una fase: solo se vuelve imprescindible cuando el jugador ha decidido llenar el espacio inicial. En fase 3, la `PRENSA DEL SUMIDERO` inaugura la compresión visible y escala el espacio a 100M, 50B y 100T para acompañar a Leucofante, plasma y Supersaiyan.

La primera `OTRA RAYITA` presenta el apelmazado antes del umbral del 70%, con un máximo introductorio de dos pedruscos. El primero revela y marca en azul la mejora de cascos. Al entrar en la segunda fase el límite sube a ocho. Seis unidades próximas salen del campo continuo y forman un pedrusco de exactamente seis unidades. Con producción tranquila se comprueba el montón cada 24 aterrizajes; por encima de dos clics por segundo baja a 18 y por encima de cuatro baja a 12. La extracción automática puede llevar la cadencia hasta un mínimo de ocho aterrizajes. Los pedruscos conservan su composición y su cantidad, bloquean la recogida y solo los cascos azules pueden tratarlos y llevarlos.

Los peones recorren un ciclo físico de recogida y transporte: salen vacíos de la caja, retiran una cantidad divisible del campo y llevan delante una mini-pila cuya superficie coincide exactamente con la carga. Al depositarla, esa misma superficie viaja hasta el almacén sin encogerse ni multiplicarse. No minan la pared; esa función pertenece al jugador y a las unidades de extracción. Peón y carga pasan a capas de primer plano durante el transporte, por lo que ningún almacén puede ocultarlos. Tras desbloquear los cascos azules, `APELMAZADO INTELIGENTE` aumenta la capacidad a ×3, ×5, ×8 y ×12, pero no crea paquetes con valor oculto: todas las unidades siguen representadas por su área real. Los pedruscos hostiles continúan siendo trabajo exclusivo de los cascos azules. `TRABAJO EN CADENA` se muestra desde el laboratorio, pero solo puede comprarse después de perforar el tabique.

Los peones blancos se dedican exclusivamente a recoger y transportar. Una run comienza sin ninguno: el primer glóbulo blanco es la primera contratación del jugador. La logística crece como una cadena visible y separada: cajón de 500, contenedor de 1.000, doble fondo de 5.000, almacén alveolar de 100.000, silo de 2 millones, silo ampliado de 10 millones, bóveda presurizada de 100 millones, bóveda hiperbárica de 1.000 millones, planta de 10.000 millones y planta de fusión de 100.000 millones. Cada salto fuerte de transporte exige primero la instalación capaz de recibirlo: el carrito avanza 12 → 60 → 300 → 1.500 unidades y el `MUGIDÓFILO DE CARGA`, 10.000 → 150.000 → 6,5 millones → 100 millones → 10.000 millones. Son mejoras numéricas de los vehículos y edificios existentes, no unidades nuevas. Con toda la logística de fase 2, la recogida ronda 479K/s frente a 471K/s de extracción combinada entre ocho púgiles y el Leucocarnero: puede vaciar el pico de producción sin chocar contra un almacén pequeño. Las ampliaciones posteriores acompañan al Leucofante y al plasma; la carga final del Supersaiyan la retira el Expreso.
...
El jugador también puede intervenir directamente en la logística. Al pulsar sobre cualquier parte ocupada del montón, se extrae de esa columna la cantidad exacta permitida por el clic; esa misma masa describe un arco hasta la caja y solo se entrega al llegar. Los pedruscos siguen necesitando cascos azules, las bacterias necesitan cuidadores y enviar una impureza manualmente ensucia la caja igual que cualquier otra entrega.

## Arquitectura para partículas masivas

`PowderField` es la única fuente de verdad para la cocaína suelta. Guarda por fosa y columna cantidades divisibles de origen jugador/Joe, no listas de bolas. `PowderSurface` convierte esa cantidad en una superficie procedural de área exacta y `PowderEffects` conserva la misma área durante caídas, recogidas y transportes. Así veinte millones de unidades necesitan un solo nodo de superficie, pero continúan ocupando matemáticamente veinte millones de píxeles.

Los peones y vehículos reclaman cantidades exactas hasta su capacidad; nunca consumen un objeto indivisible con un número interno. Todas las transferencias comparten un actualizador sin un `Tween` por unidad. `PilePiece` queda limitado a obstáculos con identidad propia —apelmazados, impurezas y bacterias—. El guardado versión 20 serializa las columnas compactas y devuelve a ellas cualquier masa que estuviera en tránsito, sin perder su origen ni duplicarla.

La prueba de regresión fija veinte mil unidades como carga de referencia: exige al menos 60 FPS, cero lotes de bolitas ordinarias, una sola superficie, búsqueda espacial acotada, transporte sin borrado cuadrático y un guardado compacto inferior a 500 KB.
