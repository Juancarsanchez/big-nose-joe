...
Cuando el jugador baja por primera vez el colocón de Joe al 70%, el apelmazado queda desbloqueado permanentemente aunque las locuras posteriores vuelvan a elevar la barra. Un grano con al menos cinco vecinos cercanos forma un pedrusco de seis unidades. Con producción tranquila se comprueba el montón cada 24 aterrizajes; por encima de dos clics por segundo baja a 18 y por encima de cuatro baja a 12. La extracción automática puede llevar la cadencia hasta un mínimo de ocho aterrizajes. Puede haber hasta ocho pedruscos simultáneos por fosa: conservan su valor, bloquean la recogida y solo los cascos azules pueden tratarlos y llevarlos. Mejorar los cascos acelera la solución, pero no aumenta el límite del problema.
...
Solo los peones blancos rascan la pared. La logística crece como una cadena visible y separada: el cajón inicial admite 1.000 unidades; el `CONTENEDOR DE NIEVE DE EMERGENCIA`, 5.000; el `SILO DE NIEVE ESTRATÉGICA`, 50.000; y la `PLANTA DE NIEVE INDUSTRIAL`, 500.000. Cada almacén conserva encima un contador local sin una segunda barra redundante. El contenedor queda deliberadamente más lejos que el cajón: la capacidad extra crea un nuevo problema de recorrido en vez de regalar también velocidad. El carrito mueve 12 unidades por viaje y enseña su carga exacta; el `REMOLQUE VESICULAR` añade un segundo vagón y eleva la carga conjunta a 36; el `MUGIDÓFILO DE CARGA` tira de tres carros con 40 y el `EXPRESO LEUCOCITARIO` recoge todo el polvo normal disponible. Ninguno mina ni daña bloques desprendidos. Si uno de esos bloques corta el paso, la logística espera hasta que el jugador lo pique.
...
Las compras de la fase 1 aparecen de forma escalonada según el trabajo entregado. `NUDILLOS DE QUERATINA` crea una ráfaga cada diez clics y `RITMO DE BAÑO` reduce más adelante los clics necesarios. Tras comprar contenedor, carrito y al menos un púgil aparecen simultáneamente el `REMOLQUE VESICULAR` y el `SINDICATO DEL PUÑO`; el jugador puede comprar ambos, pero decide cuál resuelve primero su cuello de botella. Adquirir cualquiera desbloquea la `AUTOVÍA LINFÁTICA`, una mejora única que acelera un 60% los peones y un 35% los transportes terrestres. La eficacia de limpieza de la fase inicial es mayor que la del resto para que este nuevo escalón permita alcanzar el primer desastre en aproximadamente diez minutos. La interfaz nunca identifica un cuello de botella: el jugador lo deduce viendo crecer el montón, observando cargas pequeñas o notando cómo se comporta la caja.
