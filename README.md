...
El polvo se representa como una única superficie de nieve continua. Los impactos de valor 1, 2, 3 o 4 ocupan cuatro píxeles de pantalla para que el primer minado siempre sea legible; desde valor 5, cada unidad extraída ocupa exactamente un píxel. El valor económico nunca se infla: un golpe de 1 sigue entregando 1. Por eso una descarga de 50 hace crecer la montaña 50 píxeles de masa y una descarga de 500.000 ocupa 500.000; no hay multiplicadores visuales ocultos ni una bolita gigante de valor arbitrario. Cada descarga engorda la superficie bajo su punto real de aterrizaje; las zonas cercanas se funden y las lejanas forman lomas separadas con valles, nunca torres de canicas. Los pedruscos conservan su propia capa interactiva y pueden sostener pendientes como muros de contención. El tabique actúa como un límite duro: ningún grano ni pedrusco puede atravesarlo o dibujarse por encima de la pared compacta.
...
El polvo jugable ya no crea un `Sprite2D` por bolita. Cada carga es un objeto de datos ligero, pero toda la masa ordinaria se dibuja como una única superficie procedural por fosa: veinte mil granos equivalentes no necesitan lotes de sprites ni veinte mil nodos. `RenderingServer` queda reservado para las excepciones visuales que sí deben conservar identidad propia. Los pedruscos mantienen sus grietas como nodos escasos e interactivos.
